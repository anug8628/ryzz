...
let translate (stmts) =
  ...
  and func_t  = L.function_type context
  ...
  let ltype_of_typ = function
    ...
    | A.String -> string_t
    | A.Func (params, rtype) -> 
      let formal_types = Array.of_list (List.map (fun (t,_) -> ltype_of_typ t) params) in 
      func_t (ltype_of_typ rtype) formal_types
  in
  ...
  * resulting registers to our map *)
  ...
  let lookup n = StringMap.find n !symbol_table in
  ...
  let define_func fdecl = 
    ...
    in let ftype = func_t (ltype_of_typ fdecl.srtyp) formal_types in
  ...
  let rec build_function_body fdecl =
    let _ = define_func fdecl in
    let (the_function, _) = StringMap.find fdecl.sfname !symbol_table in
    let builder = L.builder_at_end context (L.entry_block the_function) in

  let rec build_expr builder ((_, e) : sexpr) = match e with
    SNumLit i  -> L.const_float float_t i
  | SBoolLit b  -> L.const_int bool_t (if b then 1 else 0)
  | SStringLit s -> L.const_str string_t s
...
in 
  let rec build_stmt builder = function
      ...
      | SFuncDef fd -> 
        let old_symbol_table = !symbol_table in
        symbol_table := copy_map !symbol_table;
        let res = build_function_body fd in
        symbol_table := old_symbol_table;
        res
      ...
      | SFor(init, cond, inc, body) -> 
        ...
        let for_bb = L.append_block context "for" in
